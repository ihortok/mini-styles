...
The entry point loads tokens, reset, element defaults, then components. All rules live in `@layer mini`. Normal unlayered application styles can override them.
...
- Spacing tokens are `--mini-space-sm` (0.25rem), `--mini-space-md` (0.5rem), and `--mini-space-lg` (1rem). These replace `--mini-space-2`, `--mini-space-3`, and `--mini-space-4`, respectively.
